    ...
    with (support / 'collector.lock').open('a') as lock:
        ...
        for p in sorted(exports(source)) if source.exists() else []:
            ...
            try:
                ...
                folders=[d for d in root.iterdir() if d.is_dir() and re.match(r'^\d{4}-\d{2}-\d{2}_',d.name) and (p.stem==d.name or p.stem.startswith(d.name+'_'))]
                matched=max(folders,key=lambda d:len(d.name)) if folders else None
                destination=(matched/'04_최종본'/'자동보관') if matched else root/'00_자동보관_분류대기'
...
if __name__=='__main__':
    ap=argparse.ArgumentParser();ap.add_argument('--source',type=Path,default=Path.home()/'Movies/CapCut');ap.add_argument('--root',type=Path,default=Path(__file__).resolve().parents[1]);ap.add_argument('--projects',type=Path,default=Path.home()/'Movies/CapCut/User Data/Projects/com.lveditor.draft');ap.add_argument('--settle',type=int,default=60);a=ap.parse_args();run(a.source,a.root,a.projects,a.settle)
