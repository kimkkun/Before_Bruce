...
cut_audio=np.concatenate(pieces,axis=1);dest=outdir/'2026-09-11_order-list_v001-cut.mp4'
plan={'source':str(src),'source_sha256':hashlib.sha256(src.read_bytes()).hexdigest(),'source_duration':179.349342,'asr_to_source_offset_seconds':offset,'retake_removed_asr':[7.248,9.002],'fps':30,'duration':cursor/30,'clips':len(keeps),'keeps':keeps,'stage':'cut only; original framing, no captions or music','audio':'original stereo 44100 Hz; 4 ms cut-edge fades'}
(outdir/'v001-cut-plan.json').write_text(json.dumps(plan,ensure_ascii=False,indent=2));print('PLAN',cursor/30,len(keeps),flush=True)
